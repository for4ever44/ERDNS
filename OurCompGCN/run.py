...
class Runner(object):
	...
	def load_data(self):
		...
		for (sub, rel), obj in self.sr2o.items():
			for ele in obj:
				self.triples['train'].append({'triple': (sub, rel, ele), 'label': self.sr2o[(sub, rel)], 'sub_samp': 1})
	...
	def read_batch(self, batch, split):
		...
		"""
		if split == 'train':
			triple, label,tri_freq,neg_entities = [ _.to(self.device) for _ in batch]
			return triple[:, 0], triple[:, 1], triple[:, 2], label,tri_freq,neg_entities
		else:
	...
	def evaluate(self, split, epoch):
		...
		self.logger.info(
			'[Epoch {} {}]: Hit@1 {:.5}, Hit@3 : {:.5}, Hit@10 : {:.5}'.format(epoch, split, results['hits@1'],
	...
	def run_epoch(self, epoch, val_mrr = 0):
		...
		for step, batch in enumerate(train_iter):
			...
			sub, rel, obj, label, freq, neg_entities = self.read_batch(batch, 'train')
			# mark = label > 0.5
			# index = torch.nonzero(mark)[:,0]
			# real_freq = freq[index]
			aa = torch.reciprocal(torch.pow(freq, self.p.square))
			aa = aa / aa.sum()

			negative_sample_size = neg_entities.shape[1]
			N_sample = int(negative_sample_size / 6)
			ind1 = torch.arange(sub.shape[0]).cuda()
			ind2 = obj
			# ind1, ind2 = torch.where(mark)
			pos_num = len(ind1)  # N_sample = self.p.N_sample

			ss_torch_out = torch.multinomial(aa, N_sample * pos_num - pos_num, replacement=True)


			ss_torch_index, ss_count = torch.sort(ss_torch_out)



			ss_torch_rand	 = torch.randint(negative_sample_size, ss_torch_index.shape)

			neg_entities = neg_entities[ss_torch_index, ss_torch_rand]



			pred = self.model.forward(sub, rel,ss_torch_index, neg_entities, ind2, ind1)



			label = torch.cat([label[ss_torch_index,neg_entities],label[ind1,ind2]])
...
if __name__ == '__main__':
	...
	parser.add_argument('-square',		type=float,             default=1.0,			help='Margin')
	parser.add_argument('-N_sample',		type=int,             default=1024,			help='Margin')
