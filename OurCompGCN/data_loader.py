...
class TrainDataset(Dataset):
	...
	def __init__(self, triples, params):
		...
		self.N_sample = 6 * self.p.N_sample
	...
	def __getitem__(self, idx):
		...
		tri_freq = len(ele['label'])
		if self.p.lbl_smooth != 0.0:
			trp_label = (1.0 - self.p.lbl_smooth)*trp_label + (1.0/self.p.num_ent)
		#################### get negative samples ##############
		neg_samples = self.get_neg_ent(ele,label)
		neg_samples = torch.LongTensor(neg_samples)
		return triple, trp_label,tri_freq, None, None, neg_samples
	...
	def collate_fn(data):
		...
		tri_freq = torch.LongTensor([_[2] for _ in data])
		neg_entities = torch.stack([_[5] for _ in data], dim=0)
		return triple, trp_label,tri_freq, neg_entities
	...
	def get_neg_ent(self, triple, label):
		def get(triple, label):
			...
			neg_ent		= np.int32(np.random.choice(self.entities[mask], self.N_sample, replace=True)).reshape([-1])
