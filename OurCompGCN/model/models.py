...
class CompGCN_TransE(CompGCNBase):
	...
	def forward(self, sub, rel,ss_torch_index=False,ss_torch_rand=False,obj = False, pos_ind=False):

		sub_emb, rel_emb, all_ent	= self.forward_base(sub, rel, self.drop, self.drop)
		obj_emb				= sub_emb + rel_emb

		if torch.is_tensor(obj):
			s = torch.cat((ss_torch_index,pos_ind))
			o = torch.cat((ss_torch_rand,obj))

			x = self.p.gamma - torch.norm(obj_emb[s] - all_ent[o], p=1, dim=1)
		else:
...
class CompGCN_DistMult(CompGCNBase):
	...
	def forward(self, sub, rel,ss_torch_index=False,ss_torch_rand=False, obj = False, pos_ind=False):
		...
		if torch.is_tensor(obj):
			s = torch.cat((ss_torch_index,pos_ind))
			o = torch.cat((ss_torch_rand,obj))
			x = torch.bmm(obj_emb[s].unsqueeze(1), all_ent[o].unsqueeze(1).transpose(1, 2)).squeeze()
			x += self.bias[o]
		else:
...
class CompGCN_ConvE(CompGCNBase):
	...
	def forward(self, sub, rel,ss_torch_index=False,ss_torch_rand=False, obj = False, pos_ind=False):
		...
		if torch.is_tensor(obj):
			s = torch.cat((ss_torch_index,pos_ind))
			o = torch.cat((ss_torch_rand,obj))
			x = torch.bmm(x[s].unsqueeze(1),all_ent[o].unsqueeze(1).transpose(1, 2)).squeeze()
